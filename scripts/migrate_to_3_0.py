#!/usr/bin/env python3
"""Upgrade an existing Stock Workbench database to the 3.0 schema."""
...
REQUIRED_COLUMNS: dict[str, set[str]] = {
    ...
    "login_sessions": {"session_id", "login_user_id", "created_at", "expires_at", "last_seen_at"},
    "securities_accounts": {
        "id",
        "login_user_id",
        "name",
    ...
    "watchlist": {"code", "name", "login_user_id"},
    "portfolio": {"id", "code", "account_id", "total_shares", "avg_cost", "current_price"},
    "daily_pnl": {"date", "account_id", "code6", "total_assets", "cash", "pnl"},
    "analysis_reports": {"code", "signal", "login_user_id", "raw_state", "market_snapshot"},
    ...
    "holding_daily_reviews": {"review_id", "date", "account_id", "status", "portfolio_snapshot_json"},
    "holding_review_items": {
        "review_id",
        "date",
        "account_id",
        "code",
...
def _backup_database(db_path: Path) -> Path | None:
    ...
    backup_path = backup_dir / f"{db_path.stem}_pre_3_0_{stamp}{db_path.suffix or '.db'}"
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        cursor = conn.execute(
            ...
            SET status='interrupted',
                error=CASE
                    WHEN error IS NULL OR error = '' THEN '3.0 迁移时发现旧运行任务，已转为可恢复中断'
                    ELSE error || '；3.0 迁移时转为可恢复中断'
                END,
                current_code='',
                worker_id=NULL,
                heartbeat_at=NULL,
...
def migrate_database(db_path: Path | str | None = None, *, create_backup: bool = True) -> dict[str, Any]:
    ...
    if not verification["ok"]:
        raise RuntimeError(f"3.0 schema verification failed: {verification['missing']}")
    return {
        "status": "ok",
        "db_path": str(target),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upgrade Stock Workbench database to 3.0.")
