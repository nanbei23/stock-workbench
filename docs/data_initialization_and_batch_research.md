...
`AI投研中心` 的股票选择和任务中心提供三个后台任务：

- `预取数据`：批量拉取七层数据，写入 `stock_data_snapshots`
- `生成报告`：复用完整快照生成 AI 报告，写入 `analysis_reports`
- `生成组合研究`：读取选中完整报告生成组合研究方案

任务创建后页面不会被阻塞，前端会轮询 `/api/batch-research/jobs` 展示状态。服务重启或终端中断后，旧的运行中任务会被标记为 `interrupted`，不会一直卡在运行中。失败或等待项可以通过页面按钮或 API 重试。`/reports` 适合 125 份报告级别的筛选、预览、导出，并支持选中报告后生成组合研究方案。
...
组合研究方案默认只生成研究资产，不自动写入交易、持仓或交易计划，避免把研究建议直接变成操作。
