...
DEFAULT_INVESTMENT_SETTINGS = {
    ...
    "investment_position_sizing_discipline": "分三批建仓，首仓约三分之一；单只股票不超过投资风格单票上限；同板块不超过板块上限；总仓位不超过总仓位上限。",
...
def normalize_investment_profile(settings: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "max_sector_position_pct": _float_text(raw.get("investment_max_sector_position_pct"), "50"),
        "max_total_position_pct": _float_text(raw.get("investment_max_total_position_pct"), "85"),
        "max_single_trade_loss_pct": _float_text(raw.get("investment_max_single_trade_loss_pct"), "3"),
        "initial_entry_fraction": _float_text(raw.get("investment_initial_entry_fraction"), "0.333"),
        ...
        "entry_strategy_name": str(raw.get("investment_entry_strategy_name") or "").strip(),
        "entry_required_conditions": str(raw.get("investment_entry_required_conditions") or "").strip(),
        "entry_supporting_conditions": str(raw.get("investment_entry_supporting_conditions") or "").strip(),
        "buy_veto_rules": str(raw.get("investment_buy_veto_rules") or "").strip(),
        "position_sizing_discipline": str(raw.get("investment_position_sizing_discipline") or "").strip(),
        "add_position_discipline": str(raw.get("investment_add_position_discipline") or "").strip(),
...
def investment_profile_context(
    ...
    *,
    ...
    lines = [
        ...
        f"- 同板块仓位上限：{profile['max_sector_position_pct']}%",
        f"- 总仓位上限：{profile['max_total_position_pct']}%",
        f"- 最低现金保留：{profile['min_cash_pct']}%",
        f"- 可接受最大回撤：{profile['max_drawdown_pct']}%",
        f"- 单一交易最大亏损：{profile['max_single_trade_loss_pct']}%",
        f"- 首批建仓比例：{profile['initial_entry_fraction']}",
        f"- 买入触发偏好：{profile['entry_preference']}",
        f"- 入场策略名称：{profile['entry_strategy_name']}",
        f"- 入场必选条件：{profile['entry_required_conditions']}",
        f"- 入场确认/加分条件：{profile['entry_supporting_conditions']}",
        f"- 买入否决项：{profile['buy_veto_rules']}",
        f"- 仓位执行纪律：{profile['position_sizing_discipline']}",
        f"- 加仓纪律：{profile['add_position_discipline']}",
        f"- 卖出纪律：{profile['exit_discipline']}",
        f"- 左侧交易：{left_side}",
        f"- 高波动题材：{volatility}",
        "- 输出要求：最终报告必须说明“是否匹配当前投资风格”，并按上述设置字段给出风格匹配度、必选条件满足情况、确认条件满足情况、否决项、试仓条件、加仓条件、放弃条件、止损位、最大仓位和不买的核心原因。",
        '- 结构化字段要求：raw_state 或 final_decision 中尽量包含 style_match={"match_score":0-100,"match_reason":"","trial_entry":"","add_condition":"","abandon_condition":"","stop_loss":"","max_position_pct":""} 与 strategy_checklist={"required":[],"supporting":[],"veto":[],"sizing":""}。',
    ]
...
def investment_profile_snapshot(
    ...
    *,
    ...
    output_contract = {
        ...
        "strategy_checklist": {
            "required": "入场必选条件逐条是否满足",
            "supporting": "入场确认/加分条件逐条是否满足",
            "veto": "买入否决项逐条是否触发",
            "sizing": "仓位执行纪律、分批和组合约束",
        }
    }
    return {
        **profile,
...
def style_match_assessment(result: dict[str, Any] | None, profile: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "trial_entry": profile.get("entry_required_conditions") or "参考报告交易计划中的试仓触发条件；若未写明，应等待右侧确认。",
        "add_condition": profile.get("add_position_discipline") or "仅在趋势、资金和题材继续共振且未突破单票仓位上限时加仓。",
        "abandon_condition": profile.get("buy_veto_rules") or "触发报告失效条件、资金转弱、关键位破位或风格匹配度过低时放弃。",
        "stop_loss": profile.get("exit_discipline") or "以报告中的关键支撑/硬止损为准；未写明时要求重跑或人工补充。",
        "max_position_pct": profile.get("max_single_position_pct") or DEFAULT_INVESTMENT_SETTINGS["investment_max_single_position_pct"],
        "strategy_checklist": {
            "entry_strategy_name": profile.get("entry_strategy_name") or "",
            "required_conditions": profile.get("entry_required_conditions") or "",
            "supporting_conditions": profile.get("entry_supporting_conditions") or "",
            "veto_rules": profile.get("buy_veto_rules") or "",
            "position_sizing": profile.get("position_sizing_discipline") or "",
        },
    }
...
def infer_profile_from_trade_history(db_path: Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "suggested_settings": {
            ...
            "investment_entry_strategy_name": DEFAULT_INVESTMENT_SETTINGS["investment_entry_strategy_name"],
            "investment_entry_required_conditions": DEFAULT_INVESTMENT_SETTINGS["investment_entry_required_conditions"],
            "investment_entry_supporting_conditions": DEFAULT_INVESTMENT_SETTINGS["investment_entry_supporting_conditions"],
            "investment_buy_veto_rules": DEFAULT_INVESTMENT_SETTINGS["investment_buy_veto_rules"],
            "investment_position_sizing_discipline": DEFAULT_INVESTMENT_SETTINGS["investment_position_sizing_discipline"],
            "investment_add_position_discipline": DEFAULT_INVESTMENT_SETTINGS["investment_add_position_discipline"],
            "investment_max_sector_position_pct": DEFAULT_INVESTMENT_SETTINGS["investment_max_sector_position_pct"],
            "investment_max_total_position_pct": DEFAULT_INVESTMENT_SETTINGS["investment_max_total_position_pct"],
            "investment_max_single_trade_loss_pct": DEFAULT_INVESTMENT_SETTINGS["investment_max_single_trade_loss_pct"],
            "investment_initial_entry_fraction": DEFAULT_INVESTMENT_SETTINGS["investment_initial_entry_fraction"],
